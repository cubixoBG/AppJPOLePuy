...
class UtilisateurCrudTest extends KernelTestCase
{
    ...
    private function buildUtilisateur(array $overrides = []): Utilisateur
    {
        ...
        $u->setEtablissement($overrides['etablissement'] ?? 'Lycée Hugo');
    ...
    public function testCreateUtilisateurPersistsInDatabase(): void
    {
        ...
        $this->assertNotNull(
            $utilisateur->getId(),
            'L\'ID doit être généré après la persistance.'
        );
    }
    ...
    public function testReadUtilisateurById(): void
    {
        ...
        $this->em->clear(); // Vide le cache Doctrine
        $found = $this->repository->find($utilisateur->getId());

        $this->assertNotNull($found, 'L\'utilisateur doit être retrouvable par son ID.');
    ...
    {
        $found = $this->repository->find('id-inexistant-xyz');
    ...
    public function testDeleteUtilisateurRemovesFromDatabase(): void
    {
        ...
        $this->assertNull($deleted, 'L\'utilisateur supprimé ne doit plus exister en base.');
    }
    ...
    public function testDeleteDoesNotAffectOtherRecords(): void
    {
        ...
        $this->assertNotNull($this->repository->find($u1->getId()), 'u1 doit être intact.');
        $this->assertNull($this->repository->find($u2->getId()),    'u2 doit être supprimé.');
    }
}